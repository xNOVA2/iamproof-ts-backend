...
export const sendOtp = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    const user = await findUser({ email: req.body?.email }).select('+otp').select('+otpExpiresAt');
    ...
    // Set OTP expiry time to 20 minutes from now
    ...
    generateResponse(otp, 'OTP sent to email', res);
})
